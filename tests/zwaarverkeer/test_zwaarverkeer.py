...
def create_basic_auth_headers(username, password):
    ...
    auth_string = "Basic " + base64.b64encode(credentials.encode("utf-8")).decode(
        "utf-8"
    )
...
class TestDecosZwaarverkeer:
    ...
    def test_get_permits_with_a_single_permit(
        self,
        mocker,
        passage_at,
        permit_type,
        ...
        mocker.patch(
            "zwaarverkeer.views.DecosZwaarverkeer._get_response",
    ...
    def test_get_permits_with_multiple_permits(self, mocker, passage_at, permits):
        decos_response = {"count": len(permits), "content": []}
...
class TestVerkeersvergunningen:
    ...
    auth_headers = create_basic_auth_headers(
        settings.CLEOPATRA_BASIC_AUTH_USER, settings.CLEOPATRA_BASIC_AUTH_PASS
    )
    ...
    def test_no_basic_auth_credentials_supplied_by_client(self, client):
        response = client.post(
            self.URL, json.dumps(self.test_payload), content_type="application/json"
        )
