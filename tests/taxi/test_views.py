...
class TestViews:
    @patch(
        "taxi.decos.DecosTaxiDriver._get_driver_decos_key",
        lambda *args, **kwargs: mock_driver(),
    )
    @patch(
        "taxi.decos.DecosTaxiDriver._get_ontheffing",
        lambda *args, **kwargs: mock_ontheffing_driver(),
    )
    @patch(
        "taxi.decos.DecosTaxiDriver._get_handhavingzaken",
        lambda *args, **kwargs: mock_handhavingen(),
    )
    def test_ontheffingen_bsn(self, client):
        ...
        assert response.data["ontheffing"][0][
            PermitParams.ontheffingsnummer.name
        ] == str(data["ontheffingsnummer"])
        assert (
            response.data["ontheffing"][0]["schorsingen"][0][
                PermitParams.zaakidentificatie.name
            ]
    ...
    @patch(
        "taxi.decos.DecosTaxiDriver._get_ontheffing",
        lambda *args, **kwargs: mock_ontheffing_driver_empty(),
    )
    @patch(
        "taxi.decos.DecosTaxiDriver._get_handhavingzaken",
        lambda *args, **kwargs: mock_handhavingen(),
    ...
    def test_get_ontheffingen_by_driver_bsn(self, client):
        ...
        assert PermitParams.ontheffingsnummer.name in response.data["ontheffing"][0]
        assert (
            response.data["ontheffing"][0][PermitParams.ontheffingsnummer.name]
            == "8E5F8EB000EC4938BC894CA2313E9134"
        )
        assert len(response.data["ontheffing"][0]["schorsingen"]) == 2

    @patch(
        "taxi.decos.DecosTaxiDetail._get_ontheffing",
        lambda *args, **kwargs: mock_ontheffing_detail(),
    )
    @patch(
        "taxi.decos.DecosTaxiDetail._get_handhavingzaken",
        lambda *args, **kwargs: mock_handhavingen(),
    )
    def test_ontheffingen_detail(self, client):
        ...
        assert response.data[PermitParams.ontheffingsnummer.name] == str(
            kwargs["ontheffingsnummer"]
        )
        assert (
